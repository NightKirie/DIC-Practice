...
endmodule // stratixgx_ram_register
...
`timescale 1 ps/1 ps
  module stratixgx_ram_block 
   ...
   stratixgx_ram_clear ram_aclra_reg
     (
   ...
   stratixgx_ram_clear ram_aclrb_reg
     (
   ...
   stratixgx_ram_register	ram_portadatain_reg 
     (
   ...
   stratixgx_ram_register	ram_portaaddr_reg 
     (
   ...
   stratixgx_ram_register	ram_portawe_reg 
     (
      .data (portawe_delta),
      .clk (clk_bus[0]),
      .aclr(awe_aclr),
   ...
   stratixgx_ram_register	ram_portbdatain_reg 
     (
   ...
   stratixgx_ram_register	ram_portbaddr_reg 
     (
   ...
   stratixgx_ram_register	ram_portbrewe_reg 
     (
   ...
   stratixgx_ram_register	ram_portabyteenamasks_reg 
     (
   ...
   stratixgx_ram_register	ram_portbbyteenamasks_reg 
     (
   ...
   stratixgx_ram_internal internal_ram
     (
   ...
   stratixgx_ram_register	ram_portadataout_reg 
      (
   ...
   stratixgx_ram_register	ram_portbdataout_reg 
     (
...
// get the absolute value
    ...
    begin
        ...
        begin 
            ...
            if (l_pll_type == "lvds")
                i_g0_initial = counter_initial(get_phase_degree(ph_adjust(get_int_phase_shift(clk2_phase_shift, clk2_phase_shift_num), max_neg_abs)), i_m, i_n);
            else
            ...
            if (l_pll_type == "lvds")
                i_g0_ph = counter_ph(get_phase_degree(ph_adjust(get_int_phase_shift(clk2_phase_shift, clk2_phase_shift_num),max_neg_abs)), i_m, i_n);
            else
            ...
            begin
                ...
                begin
                    if (m_mode_val === "bypass")
                        ...
                        $display("                                   M modulus = %d ", m_val_tmp[9:0]);
                    m_mode_val = "";
                end
            end
            ...
            begin
                ...
                begin
                    if (n_mode_val === "bypass")
                        $display ("Warning : N counter switched from BYPASS mode to enabled (N modulus = %d). PLL may lose lock.", n_val[9:0]);
                    else
...
begin
    ...
    begin
        $display (" Note : Stratixgx PLL was reset");
        $display ("Time: %0t  Instance: %m", $time);
    end
    ...
    begin
        $display (" Note : Stratixgx PLL was disabled");
        $display ("Time: %0t  Instance: %m", $time);
    end
       ...
       begin
          ...
          begin
              ...
              begin
                  ...
                  begin
                      ...
                      pll_about_to_lock = 0;
                      cycles_to_lock = 0;
                      $display ("Note : Stratixgx PLL lost lock");
                      $display ("Time: %0t  Instance: %m", $time);
                      first_schedule = 1;
                      schedule_offset = 1;
                      vco_period_was_phase_adjusted = 0;
          ...
          begin
              ...
              begin
                  if (no_warn == 0)
                  begin
                      $display("Warning : Incoming clock period %d for Stratixgx PLL does not match the specified inclock period %d. ALTGXB simulation may not function correctly.", refclk_period, primary_clock_frequency);
                      $display ("Time: %0t  Instance: %m", $time);
                      no_warn = 1;
                  end
              end
    ...
    begin
       ...
       begin
           ...
           begin
              ...
              if (l_pll_type == "fast")
                 locked_tmp = 1;
              $display ("Note : Stratixgx PLL lost lock due to loss of input clock");
              $display ("Time: %0t  Instance: %m", $time);
           end
           pll_about_to_lock = 0;
           cycles_to_lock = 0;
...
//       else if (abs(fbclk_time - refclk_time) <= 2 || (refclk_period - abs(fbclk_time - refclk_time) <= 2))
       ...
       begin
           ...
           begin
              ...
              begin
                  $display (" Note : Stratixgx PLL locked to incoming clock");
                  $display ("Time: %0t  Instance: %m", $time);
              end
              pll_is_locked = 1;
              locked_tmp = 1;
...
module stratixgx_lvds_tx_parallel_register (clk, enable, datain, dataout, devclrn, devpor);
input [9:0] datain;
input clk;
input enable;
input devclrn;
...
module stratixgx_lvds_tx_out_block (clk, datain, dataout, devclrn, devpor);
input datain;
input clk;
input devclrn;
input devpor;
...
stratixgx_lvds_tx_out_block output_module (.clk(clk0_dly2),
													  .datain(shift_out),
													  .dataout(dataout),
													  .devclrn(devclrn),
													  .devpor(devpor));
...
stratixgx_lvds_tx_parallel_register input_reg (.clk(txload0_dly),
															.enable(1'b1),
															.datain(datain_dly),
															.dataout(input_data),
															.devclrn(devclrn),
...
`timescale 1 ps/1 ps
module  stratixgx_jtag (tms, tck, tdi, ntrst, tdoutap, tdouser, tdo, tmsutap, tckutap, tdiutap, shiftuser, clkdruser, updateuser, runidleuser, usr1user);
...
buf (clk0_in_pre, clk0);
buf (coreclk_in_pre, coreclk);
buf (datain_in, datain);
buf (reset_in, reset);

specify
   (posedge clk0 => (dataout[0] +: data_out[0])) = (0, 0);
   (posedge clk0 => (dataout[1] +: data_out[1])) = (0, 0);
   (posedge clk0 => (dataout[2] +: data_out[2])) = (0, 0);
   (posedge clk0 => (dataout[3] +: data_out[3])) = (0, 0);
   (posedge clk0 => (dataout[4] +: data_out[4])) = (0, 0);
   (posedge clk0 => (dataout[5] +: data_out[5])) = (0, 0);
   (posedge clk0 => (dataout[6] +: data_out[6])) = (0, 0);
   (posedge clk0 => (dataout[7] +: data_out[7])) = (0, 0);
   (posedge clk0 => (dataout[8] +: data_out[8])) = (0, 0);
   (posedge clk0 => (dataout[9] +: data_out[9])) = (0, 0);
endspecify
...
always @(clk0_in_pre or coreclk_in_pre)
begin
   clk0_in = clk0_in_pre;
...
begin
   ...
   begin
      ...
      if (sync_reset == 1'b1) 
          dataout_tmp[channel_width-1:0] = 1'b0;
...
integer i;
reg [9:0] ram_datain;
wire [9:0] ram_dataout;

reg  	[1:0]  wrPtr,rdPtr;	// writer pointer, read pointer
...
specify
  	...
  	(posedge rclk => (dataout[0] +: data_out[0])) = (0, 0);
  	(posedge rclk => (dataout[1] +: data_out[1])) = (0, 0);
  	(posedge rclk => (dataout[2] +: data_out[2])) = (0, 0);
  	(posedge rclk => (dataout[3] +: data_out[3])) = (0, 0);
  	(posedge rclk => (dataout[4] +: data_out[4])) = (0, 0);
  	(posedge rclk => (dataout[5] +: data_out[5])) = (0, 0);
  	(posedge rclk => (dataout[6] +: data_out[6])) = (0, 0);
  	(posedge rclk => (dataout[7] +: data_out[7])) = (0, 0);
  	(posedge rclk => (dataout[8] +: data_out[8])) = (0, 0);
  	(posedge rclk => (dataout[9] +: data_out[9])) = (0, 0);
...
always @(posedge wclk_in or posedge sync_reset) 
begin
	if(sync_reset == 'b1) 
	begin
      wrPtr <= 0;
	...
	if (reset_in)
		...
		begin
			...
			if (slip_count == 0)
				...
				begin
					j = 0;
					for (i = (channel_width - slip_count); i < channel_width; i = i + 1)
						begin
...
wire [9 : 0] fifo_datain;
wire fifo_wclk;
wire fifo_rclk;

wire [9 : 0] fifo_dataout;

// input/output of RXPDAT2
wire [9 : 0] rxpdat2_datain;
wire [9 : 0] rxpdat2_dataout;

// input/output of RXPDAT3
wire [9 : 0] rxpdat3_datain;
wire [9 : 0] rxpdat3_dataout;
...
module stratixgx_nondpa_lvds_receiver (clk0, enable0, enable1, datain, dataout, devclrn, devpor);
input datain;
input clk0;
input enable0;
input enable1;
...
stratixgx_lvds_rx_parallel_register load_reg (.clk(!clk0),
															.enable(rxload2_dly),
															.datain(shift_data),
															.dataout(load_data),
															.reset(1'b0),
...
stratixgx_lvds_rx_parallel_register output_reg (.clk(txload_out),
																.enable(1'b1),
