...
begin
    ...
    begin
        $display (" Note : Cyclone PLL was reset");
        $display ("Time: %0t  Instance: %m", $time);
    end
    ...
    begin
        $display (" Note : Cyclone PLL was disabled");
        $display ("Time: %0t  Instance: %m", $time);
    end
       ...
       begin
          ...
          begin
              ...
              begin
                  ...
                  begin
                      ...
                      pll_about_to_lock = 0;
                      cycles_to_lock = 0;
                      $display ("Note : Cyclone PLL lost lock");
                      $display ("Time: %0t  Instance: %m", $time);
                      first_schedule = 1;
                      schedule_offset = 1;
                      vco_period_was_phase_adjusted = 0;
          ...
          begin
              ...
              begin
                  if (no_warn == 0)
                  begin
                      $display("Warning : Incoming clock period %d for Cyclone PLL does not match the specified inclock period %d. ALTGXB simulation may not function correctly.", refclk_period, primary_clock_frequency);
                      $display ("Time: %0t  Instance: %m", $time);
                      no_warn = 1;
                  end
              end
    ...
    begin
       ...
       begin
           ...
           begin
              ...
              if (l_pll_type == "fast")
                 locked_tmp = 1;
              $display ("Note : Cyclone PLL lost lock due to loss of input clock");
              $display ("Time: %0t  Instance: %m", $time);
           end
           pll_about_to_lock = 0;
           cycles_to_lock = 0;
...
//       else if (abs(fbclk_time - refclk_time) <= 2 || (refclk_period - abs(fbclk_time - refclk_time) <= 2))
       ...
       begin
           ...
           begin
              ...
              begin
                  $display (" Note : Cyclone PLL locked to incoming clock");
                  $display ("Time: %0t  Instance: %m", $time);
              end
              pll_is_locked = 1;
              locked_tmp = 1;
...
module cyclone_io (datain, oe, outclk, outclkena, inclk, inclkena, areset, sreset,
	...
	// input reg
	cyclone_mux21 inreg_D_mux (.MO (inreg_D),
			           .A (padio),
			           .B (inreg_sreset),
	...
	//output reg
	cyclone_mux21 outreg_D_mux (.MO (outreg_D),
			           .A (datain),
			           .B (outreg_sreset),
	...
	//oe reg
	cyclone_mux21 oereg_D_mux (.MO (oereg_D),
			           .A (oe),
			           .B (oereg_sreset),
